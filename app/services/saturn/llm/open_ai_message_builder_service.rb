class Saturn::Llm::OpenAiMessageBuilderService
  ...
  def attachment_parts(attachments)
    ...
    attachment_part = text_part('Kullanıcı bir ek paylaştı') if attachments.where.not(file_type: %i[image audio]).exists?
